...
public class EventExtractor {
	...
	public static String formatDate(String timestring) {
		...
		DateTime datetime = ISODateTimeFormat.dateTimeNoMillis().parseDateTime(
				timestring);
		DateTimeFormatter fmt = DateTimeFormat
				.forPattern("EEEE, MMMM d, YYYY h:mm a");
	...
	public static List<Entity> retrieve(String sortKey, String sortDirection,
			int limit) {
		SortDirection dir = (sortDirection.equalsIgnoreCase("ascending")) ? SortDirection.ASCENDING
				: SortDirection.DESCENDING;
		Query query = new Query("event").addSort(sortKey, dir);
		return retrieveHelper(query, limit);
	}

	public static List<Entity> filter(String property, String keyword,
			String sortDirection, int limit) {
		SortDirection dir = (sortDirection.equalsIgnoreCase("ascending")) ? SortDirection.ASCENDING
				: SortDirection.DESCENDING;
		Filter filter = new FilterPredicate(property, FilterOperator.EQUAL,
				keyword);
		Query query = new Query("event").setFilter(filter).addSort(
				"start_time", dir);
		return retrieveHelper(query, limit);
	}
	...
	public static List<Entity> findEventsToday() {
		...
		for (Entity e : allEvents) {
			if (spansToday((String) e.getProperty("start_time"),
					(String) e.getProperty("end_time"))) {
				result.add(e);
			}
		}
		return result;
	...
	/**
	 * Returns the events falling within the number of upcoming weeks from
	 * current date.
	 * 
	 * @param numWeeks
	 * @return
	...
	private static List<Entity> retrieveHelper(Query query, int limit) {
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
		List<Entity> events = datastore.prepare(query).asList(
	...
	private static boolean isInRange(String otherDate, int numWeeks) {
		...
		if (otherDate.length() == 10) {
			...
			dateTime1 = ISODateTimeFormat.dateTimeNoMillis().parseDateTime(
					otherDate);
		}
	...
	private static boolean isToday(String otherDate) {
		...
		return (d1.getDayOfYear() == d2.getDayOfYear() && d1.getYear() == d2
				.getYear());
	}
